...
function ChatHistory({ setMessages,setSelectedChatId,setSelectedChatTitle,onNewChat }: ChatHistoryProps) {
  ...
  const handleSelectChat = async(chatId:number,chatTitle:string) => {
    ...
    try {
      const response = await axios.get(`https://zaid-backend-4.onrender.com/assistant/chat-messages?user_id=${userId}&chat_id=${chatId}`);
  ...
  const fetchChats = async () => {
    const userId = 1
        try {
            // setLoading(true);
            const response = await axios.get(`https://zaid-backend-4.onrender.com/assistant/chat-list?user_id=${userId}`);
