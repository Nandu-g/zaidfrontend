...
function Navbar({ setSelectedChatId, setMessages,selectedChatTitle }: NavbarProps) {
  ...
  const handleCloseModal = () => {
   ...
   const handleSelectSearchResult = async (chatId: number) => {
    ...
    try {
      const res = await axios.get(`https://zaid-backend-4.onrender.com/assistant/chat-messages?user_id=${userId}&chat_id=${chatId}`);
  ...
  try{
    ...
    const results = await axios.get(`https://zaid-backend-4.onrender.com/assistant/search-messages?q=${value}&user_id=${userId}`)
